...
bool ValidateSingleton(PIX *pix, BOX *box, PIX *pix8, l_float32 *pconf,
  ...
  //if(box->x > 190 && box->x < 210)
  // 	  debug = true;

  if(debug) {
	  LOGE("single 1 %d,%d,%d,%d", box->x, box->y, box->w, box->h);
  }
...
bool ValidatePairV(BOX *b1, BOX *b2, l_float32 *pconf,
  ...
  *pconf = 0.0;
  ...
  l_float32 w_ratio = RelativeDiff(b1->w, b2->w);
  ...
  //if(b1->x > 190 && b1->x < 210 && b2->x > 190 && b2->x < 210)
//	  debug = true;

  if(debug) {
	  LOGE("pairing 1 %d,%d,%d,%d", b1->x, b1->y, b1->w, b1->h);
	  LOGE("pairing 2 %d,%d,%d,%d", b2->x, b2->y, b2->w, b2->h);
  }

  /* Vertical spacing less than 2x taller edge */
  if (v_dist > params.pair_h_dist_ratio * max_w)
    return false;
  if(debug)
	  LOGE("pass vertical spacing");

  /* Must share at least 0.25x the larger horizontal edge */
  if (h_dist > 0 || L_ABS(h_dist) < max_w * params.pair_h_shared) {

	  if(debug)
		  LOGE("horizontal edge %d < %d or >0(no overlap)", h_dist, max_w * params.pair_h_shared);
    return false;
  }

  /* Widths must be at least 2x tolerance */
  if (w_ratio > params.pair_h_ratio)
    return false;
  if(debug)
	  LOGE("pass widths tolerance: %f", d_ratio);
...
bool ValidateClusterPairV(BOX *b1, BOX *b2, bool *too_far, l_float32 *pconf,
  ...
  *pconf = 0.0;
  ...
  //if(b1->x > 150 && b1->x < 200 && b2->x > 150 && b2->x < 200)
//	  debug = true;
